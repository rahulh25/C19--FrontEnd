export const LOADING_JOBS = "LOADING_JOBS";
export const LOADED_JOBS = "LOADED_JOBS";
export const JOBS_ERROR = "JOBS_ERROR";
export const REGISTRATION_SUCCESS = "REGISTRATION_SUCCESS";
export const REGISTRATION_FAILURE = "REGISTRATION_FAILURE";
export const NETWORK_ERROR = "NETWORK_ERROR";
export const VALID_EMAIL = "VALID_EMAIL";
export const INVALID_EMAIL = "INVALID_EMAIL";
export const DOES_NOT_EXISTS = "DOES_NOT_EXISTS";
export const ALREADY_EXISTS = "ALREADY_EXISTS";
export const RESET_REGISTRATION = "RESET_REGISTRATION";
export const GET_USER_SUCCESS = "GET_USER_SUCCESS";
export const GET_USER_FAILURE = "GET_USER_FAILURE";
export const RESET_USERINFO = "RESET_USERINFO";
export const UPDATE_SORTBY = "UPDATE_SORTBY";
export const LOADED_CONTRIBUTOR = "LOADED_CONTRIBUTOR";
export const LOADING_CONTRIBUTOR = "LOADING_CONTRIBUTOR";
export const CONTRIBUTOR_ERROR = "CONTRIBUTOR_ERROR";
